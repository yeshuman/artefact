...
async def test_satori_entity_detection_real(test_mondo, test_message, openai_client):
    ...
    # Create concept archetype with real embedding
    description = "Abstract philosophical concepts and ideas"
    embedding_response = await openai_client.embeddings.create(
        model="text-embedding-ada-002",
        input=f"concept: {description}"
    )
    ...
    # Create reference entities with real embeddings
    for concept in ["wisdom", "understanding", "meditation", "mindfulness"]:
        embedding_response = await openai_client.embeddings.create(
            model="text-embedding-ada-002",
            input=concept
        )
    ...
    detected_entities = [
        entity async for entity in Entity.objects.filter(message=response).select_related('archetype')
    ]
    
    # Log detected entities for debugging
    for entity in detected_entities:
        print(f"Detected entity: {entity.text} ({entity.archetype.name})")
    
    # Verify entity markup in response
    assert "<entity" in response.content, "No entity markup in response"
    
    # Verify at least one concept was detected
    concept_entities = [e for e in detected_entities if e.archetype == concept_archetype]
    assert len(concept_entities) > 0, "No concept entities detected"
    
    # Verify confidence scores
    for entity in detected_entities:
        assert entity.confidence > 0.5, f"Low confidence for entity: {entity.text}" 